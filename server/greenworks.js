...
let greenworkLibPath = '';
if (process.platform === 'darwin') {
  if (process.arch === 'x64') {
    greenworkLibPath = './lib/greenworks-osx64.node';
  }
} else if (process.platform === 'win32') {
  if (process.arch === 'x64') {
    greenworkLibPath = './lib/greenworks-win64.node';
  } else if (process.arch === 'ia32') {
    greenworkLibPath = './lib/greenworks-win32.node';
  }
} else if (process.platform === 'linux') {
  if (process.arch === 'x64') {
    greenworkLibPath = './lib/greenworks-linux64.node';
  } else if (process.arch === 'ia32') {
    greenworkLibPath = './lib/greenworks-linux32.node';
  }
}

const module = { exports: {} };
process.dlopen(module, path.resolve(greenworkLibPath), os.constants.dlopen.RTLD_NOW);

const greenworks = module.exports;
...
greenworks.ugcPublish = function(file_name, title, description, image_name, success_callback, error_callback, progress_callback) {
  const publish_file_process = function() {
    if (progress_callback) {
...
greenworks.ugcPublishUpdate = function(published_file_id, file_name, title, description, image_name, success_callback, error_callback, progress_callback) {
  const update_published_file_process = function() {
    if (progress_callback) {
      progress_callback('Completed on sharing files.');
    }

    greenworks.updatePublishedWorkshopFile(published_file_id, file_name, image_name, title, description,
      function() {
        success_callback();
      }, function(err) {
        error_process(err, error_callback);
